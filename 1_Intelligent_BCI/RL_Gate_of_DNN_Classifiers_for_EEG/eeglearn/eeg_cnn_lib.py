...
def train(images, labels, fold, model_type, mark=0, batch_size=32, num_epochs=5):
    """
    ...
    for epoch in range(num_epochs):
        ...
        if av_val_acc > best_validation_accu:
            ...
            np.savez('weights_lasg_{}_{}'.format(model_type, mark), *lasagne.layers.get_all_param_values(network))
    print('-'*50)
...
#    train(images, np.squeeze(feats[:, -1]) - 1, fold_pairs[2], 'cnn')
    ...
    mark = 0 
    print('\n')
    print('Training the 1d-CONV Model...')
    train(images_timewin, np.squeeze(feats[:, -1]) - 1, fold_pairs[mark], '1dconv', mark)
    print('Done!\n')


    print('Training the CONVPOOL Model...')
    train(images_timewin, np.squeeze(feats[:, -1]) - 1, fold_pairs[mark], 'maxpool', mark)
    print('Done!\n')

    print('Training the LSTM Model...')
    train(images_timewin, np.squeeze(feats[:, -1]) - 1, fold_pairs[mark], 'lstm', mark)
    print('Done!\n')

    print('Training the LSTM-CONV Model...')
    train(images_timewin, np.squeeze(feats[:, -1]) - 1, fold_pairs[mark], 'mix', mark)
    print('Done!')
    print('\n')
