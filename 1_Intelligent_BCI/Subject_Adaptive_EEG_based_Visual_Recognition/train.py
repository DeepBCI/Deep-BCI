...
if __name__ == "__main__":
	...
	for split_no in range(config.num_splits):
		...
		mmd = MaximumMeanDiscrepancy(kernel_type='rbf', normalize=False)
		print(net)
		...
		for n in range(1, config.num_epochs + 1):
			...
			for step in range(max_len):
				...
				for i in range(1, config.num_subjects):
					...
					yid = torch.cat((yid, yid_tmp), dim=0)

					idx_lst.append(idx_lst[-1] + x_tmp.shape[0])

				x_tr, y_tr, yid = x_tr.cuda(), y_tr.cuda(), yid.cuda()
				optimizer.zero_grad()
				out, feat = net(x_tr)
				...
				dist = 0
				for i in range(config.num_subjects):
					for j in range(i + 1, config.num_subjects):
						dist += mmd(feat[idx_lst[i]:idx_lst[i+1]], feat[idx_lst[j]:idx_lst[j+1]])

				dist /= (config.num_subjects * (config.num_subjects - 1) / 2)
			...
			with torch.no_grad():
				...
				print('{:04d} epoch train loss: {:.4f} test loss: {:.4f}'.format(n, float(loss_tr/ntr),float(loss_te/nte)))
