function batch_parameter_recovery_simul(ktov)
...
for i = 1 : 1 : 10
    disp(['M' num2str(ktov), '_S' num2str(i)]);
    T = readNPY(['simul_results/T3/MODEL' num2str(ktov) sprintf('/SUB%03d_SIMUL_BHV.npy',i) ]);
    dat = [];
    for sei = 1 : 1 : size(T,1)
        dat= [dat ; reshape(T(sei,:,:),[size(T,2),size(T,3)])];
    end
    act = dat(:,8);
    glc = dat(:,18);
    unc = ones(size(glc));
    ...
    [~ , opti0] = opt_normalization_1st2ndstage_exnan_simul(1, dat(1,7), dat(1,18), dat(1,3)); 
    prev_opti1 = opti0;
    
    prev_action_sta1 = dat(1,7);
    if dat(1,5)==4
        prev_action_sta4 = dat(1,8);
        [~, opti04] = opt_normalization_1st2ndstage_exnan_simul(dat(1,5), dat(1,8), dat(1,18), dat(1,3)); 
        prev_opti4 = opti04;
    ...
    [~, opti02] = opt_normalization_1st2ndstage_exnan_simul(dat(1,5), dat(1,8), dat(1,18), dat(1,3));
    prev_opti2 = opti02;
    ...
    for k = 2 : 1: length(dat)
        ...
        % sta1
        [opti_likeli1, opti1] = opt_normalization_1st2ndstage_exnan_simul(1, dat(k,7), dat(k,18), dat(k,3)); 
        curr_opti1 = opti1;
        ...
        if dat(k,5) == 4
            if prev4on == 0
                ...
                [~, opti04] = opt_normalization_1st2ndstage_exnan_simul(dat(k,5), dat(k,8), dat(k,18), dat(k,3)); 
                prev_opti4 = opti04;
                prev4on = 1;
            else
                prev4on = 1;
                curr_action_sta4 = dat(k,8);
                [opti_likeli4, opti4] = opt_normalization_1st2ndstage_exnan_simul(dat(k,5), dat(k,8), dat(k,18), dat(k,3)); 
                curr_opti4 = opti4;
        ...
        % stage2
        [opti_likeli2, opti2] = opt_normalization_1st2ndstage_exnan_simul(dat(k,5), dat(k,8), dat(k,18), dat(k,3)); 
        curr_opti2 = opti2;
...
% betas_colike_w_sta2_inter = [];
save(['betas_colike_w_sta2_inter__SIM' num2str(ktov) '_nanctrl_updated2.mat'],'betas_colike_w_sta2_inter');
% save(['simul_summary/T_ORI/BEHAV/betas_colike_wo_act_SIMUL' num2str(ktov) '.mat'],'betas_colike_w_sta2');
% end