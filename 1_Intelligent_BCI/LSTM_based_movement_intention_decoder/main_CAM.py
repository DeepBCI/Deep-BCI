...
def train_model(m1_sta1dat, m2_sta1dat, full_label_m1, full_label_m2, save_direc):
    ...
    m1_sta1dat = eval ( 'ext_' + meta_json["FEATURE_TO_USE"].lower() + '(m1_sta1dat)' )
    m2_sta1dat = eval ( 'ext_' + meta_json["FEATURE_TO_USE"].lower() + '(m2_sta1dat)' )
    ...
    if meta_json["SCREENING"] is False: # CNN, DNN etc
        ...
        accuracy_tw = []
        m1_sta1dat = np.concatenate ( (m1_sta1dat, m2_sta1dat), axis=0)
        m2_sta1dat = np.concatenate ( (full_label, full_label2), axis=0)

        accuracy_list = []
        epoch_i = 0

        len_dat = m1_sta1dat.shape[0]
        randind = np.random.permutation(np.linspace(0,len_dat-1,num = len_dat)).astype(dtype=np.int16)
        tr_ind = int(len_dat/10)

        kf = KFold(n_splits=10, shuffle=True)
        kf.get_n_splits(m1_sta1dat)
        ind_rp = 0
        for train_ind, test_ind in kf.split(m1_sta1dat):
            X_train, X_test = m1_sta1dat[train_ind,:,:,:], m1_sta1dat[test_ind,:,:,:]
            y_train, y_test = m2_sta1dat[train_ind], m2_sta1dat[test_ind]
            meta_json["rest"] = False
            ind_rp += 1
            print('**[{0}'.format(X_train.shape[0]) + '] samples and [{0}'.format(ind_rp) +'] CV')
            ...
            filename1 = save_direc + '/acc_{0}'.format(ind_rp)
...
if __name__ == '__main__':
    ...
    if meta_json["SUBWISE"] is True:
        ...
        accuracy_tw = train_model(ep, ep2, lb, lb2, save_direc)
