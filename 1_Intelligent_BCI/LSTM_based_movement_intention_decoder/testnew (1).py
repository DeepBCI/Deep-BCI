...
tf.app.flags.DEFINE_string('dataset', 'EEGLearn',
                           """Currently support EEGLearn dataset.""")
tf.app.flags.DEFINE_string('data_path', '', """Root directory of data""")
tf.app.flags.DEFINE_string('eval_dir', './tmp/bichen/logs/eval',
                            """Directory where to write event logs """)
tf.app.flags.DEFINE_string('checkpoint_path', './tmp/bichen/logs/train',
                            """Path to the training checkpoint.""")
tf.app.flags.DEFINE_integer('eval_interval_secs', 60 * 1,
                             """How often to check if new cpt is saved.""")
tf.app.flags.DEFINE_boolean('run_once', False,
                             """Whether to run eval only once.""")
tf.app.flags.DEFINE_string('train_dir', './tmp/bichen/logs/train',
                            """Directory where to write event logs """
                            """and checkpoint.""")
...
"""Number of steps to save summary.""")
tf.app.flags.DEFINE_string('net', 'EEGLearn',
...
def build_convpool_lstm(input_vars, nb_classes, grad_clip=110, imsize=32, n_colors=3, n_timewin=3, save_v={}):
    ...
    if 'lstm_w0' not in save_v:
        save_v['lstm_w0'] = weight_variable([n_hidden, 256])
    ...
    convpool = tf.add(tf.matmul(convpool, save_v['lstm_w1']), save_v['lstm_b1'], name = 'network')
    return convpool
...
def build_convpool_mix(input_vars, nb_classes, grad_clip=110, imsize=32, n_colors=3, n_timewin=3, n_filter=64):
    ...
    convpool = tf.add(tf.matmul(convpool, weight_variable([512, nb_classes])), bias_variable([nb_classes]), name = 'network')
...
def iterate_minibatches(inputs, targets, batchsize, shuffle=False, batchfix=True):
    """
    ...
    print('inlen:', input_len)
    if batchfix:
        emptyadd_N = batchsize-(input_len%batchsize)
        empty_shape = [1]
        empty_input = np.zeros(list(inputs[0].shape))
        empty_input = np.expand_dims(empty_input, axis=0)
        empty_target = np.zeros(list(targets[0].shape), dtype=np.int)
        empty_target = np.expand_dims(empty_target, axis=0)
        #print(inputs
        #print(targets)
        for i in range(emptyadd_N):
            #print(inputs.shape, empty_input.shape)
            inputs = np.concatenate((inputs, empty_input), axis=0)
            targets = np.concatenate((targets, empty_target), axis=0)
...
def train(images, labels, fold, model_type, batch_size=32, num_epochs=5, n_layer = (4, 2, 1)):
    ...
    with tf.Graph().as_default():
        assert FLAGS.net == 'EEGLearn', 'Selected neural net architecture not supported: {}'.format(FLAGS.net)

        if FLAGS.net == 'EEGLearn':       
            mc = eeglearn_config()
            mc.IS_TRAINING = True
        ...
        if len(images.shape) == 4:
            ...
            input_var = tf.placeholder(tf.float32, [batch_size, images_height, images_width, colorN])
            output_var = tf.placeholder(tf.float32, [batch_size, num_classes])
            ...
            print('5:', images.shape)
            input_var = tf.placeholder(tf.float32, [batch_size, windowN, images_height, images_width, colorN])
            output_var = tf.placeholder(tf.float32, [batch_size, num_classes])
        else:
            print('warning!')
            return False
        ...
        #evaluate model
        correct_pred = tf.equal(tf.argmax(network, 1), tf.argmax(output_var, 1), name='corr_pred')
        ...
        filesave_path = filepath + model_type
        ...
        with tf.Session(config = config) as sess:
            ...
            for epoch in range(num_epochs):
                ...
                for batch in iterate_minibatches(X_val, y_val, batch_size, shuffle=False):
                    inputs, targets = batch
                    targets = np.eye(num_classes)[targets.reshape(-1)]
                    sess.run(optimizer, feed_dict = {xin: inputs, yout: targets, keep_prob: 1.0})
            ...
            saver.export_meta_graph(filesave_path + '.meta')

            return av_test_acc, best_val_acc



#def pred_sample(images, labels, fold, model_type, batch_size=32, num_epochs=5, n_layer = (4, 2, 1)):
def pred_sample(images, labels, model_type, batch_size=32):
    print('Pred sample for model type:', model_type)
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config = config) as sess:
            ...
            new_saver = tf.train.import_meta_graph(filesave_path_meta, clear_devices=True)
            new_saver.restore(sess, tf.train.latest_checkpoint(filepath))
            
            graph = tf.get_default_graph()
            network = graph.get_tensor_by_name("network:0")
            correct_pred = tf.equal(tf.argmax(network, 1), tf.argmax(output_var, 1))

            #op_to_restore = graph.get_tensor_by_name("corr_pred:0")
            #input_var = [None, height, width, colorN]
            #output_var = [None, num_classes]
            corr_list = []
            ...
            for i in range(sampleN):
                #print(i)
                insample = X_[i].reshape(1, windowN, images_height, images_width, colorN)
                outsample = np.eye(num_classes)[y_[i].reshape(-1)]
                print('in:',  insample.shape)
                print('out:', outsample.shape)
                corr = sess.run(correct_pred, feed_dict = {xin: insample, yout: outsample, keep_prob: 1.0})
                corr_list.append(corr)
        
        print('Prediction finished:'+model_type)
...
if __name__ == '__main__':
    from utils import reformatInput, reformatInput_for_predict
    import pickle    
...
#
    ...
    print(fold_pairs_test[2])
    corr_1dconv = pred_sample(images_timewin, np.squeeze(feats[:, -1]) - 1, '1dconv')
    #corr_lstm = pred_sample(images_timewin, np.squeeze(feats[:, -1]) - 1, 'lstm')
    #corr_mix = pred_sample(images_timewin, np.squeeze(feats[:, -1]) - 1, 'mix')

    print(corr_1dconv[0:10])
    #print(corr_lstm[0:10])
    #print(corr_mix[0:10])
