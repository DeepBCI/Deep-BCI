...
Temp1 = CM(CM_idx);
Temp2 = sort(Temp1);
Start = length(Temp2)*1/10;
End = length(Temp2)*9/10;
...
use_CMthresh=0;
if ndims(CM)==3
if ~isfield(config,'thresh');       config.thresh=linspace(Temp2(Start),Temp2(End),20); end
% config.thresh=linspace(0,median(CM(CM_idx)),50); end
elseif ndims(CM)==4
...
%             'GT = modularity_und(CM);',... 
%             'GT = eigenvector_centrality_und(CM);'}; 
    end
else
