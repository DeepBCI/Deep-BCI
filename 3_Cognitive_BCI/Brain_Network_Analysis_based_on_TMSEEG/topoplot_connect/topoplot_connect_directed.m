...
cM = colormap;
if ~isfield(displayStruct, 'connectStrength') % no weighted graph
...
for kk = 1:numChanPairs % 이 곳이 line그려지는 곳
    plot3(y(displayStruct.chanPairs(kk, :)), x(displayStruct.chanPairs(kk, :)), [ELECTRODE_HEIGHT, ELECTRODE_HEIGHT], 'LineWidth', 2, 'Color', cM(cmapPos(kk), :));
    	 arrowh(y(displayStruct.chanPairs(kk, :)), x(displayStruct.chanPairs(kk, :)),cM(cmapPos(kk), :),[200, 200]);            % passing entire vectors
end
% praneeth - ends
