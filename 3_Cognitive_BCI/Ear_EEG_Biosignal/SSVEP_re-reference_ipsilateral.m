...
for sub=1:15
    ...
    for ch=1:8
        Rear_ch{ch} = bp(1:3750,10+ch,:)-bp(1:3750,19,:);
        Lear_ch{ch} = bp(1:3750,ch,:)-bp(1:3750,9,:);
    end
    for ch=1:3
    ...
    for ch=1:3
        ...
        SNR(sub,ch+16) = pS_avr{ch}(6,sub)/mean(pS_avr{ch}([1:5,7:11],sub),1);
    end
end
...
for ch=1:3
    ...
    SNR_avr2(1,ch+16) = pS_gavr{ch}(6,1)/mean(pS_gavr{ch}([1:5,7:11],1),1);    
end
...
%     errorbar(8.3,SNR_avr(1,ch+8),NaN,SNR_err(1,ch+8),'k')
    errorbar(8.3,SNR_avr(1,ch+8),SNR_err(1,ch+8),SNR_err(1,ch+8),'k')
    text(7.5,SNR_avr(1,ch+8)+SNR_err(1,ch+8)+0.5,num2str(round(SNR_avr(1,ch+8),2)),'FontWeight','bold','FontSize',8)
end
...
%     line([10 10],[0 pS_gavr{ch}(6,1)],'LineWidth',1, 'Color', 'r')
    ...
    ax2.YAxis.Visible = 'on'; ax2.XAxis.Visible = 'off'; ax2.FontWeight = 'bold'; ax2.YColor = 'k'; ax2.FontSize = 8;
    ax2.XAxis.Limits = [0 10]; ax2.YAxis.Limits = [0 4]; ax2.YTick = [0 2 4]; hold on;
    bar(8.3,SNR_avr(1,ch+16),1,'FaceColor',[0.9290 0.6940 0.1250]);
%     errorbar(8.3,SNR_avr(1,ch+16),NaN,SNR_err(1,ch+16),'k')
    errorbar(8.3,SNR_avr(1,ch+16),SNR_err(1,ch+16),SNR_err(1,ch+16),'k')
    text(7.5,SNR_avr(1,ch+16)+SNR_err(1,ch+16)+0.5,num2str(round(SNR_avr(1,ch+16),2)),'FontWeight','bold','FontSize',8)
end
supt = suptitle('SSVEP : Ipsilateral - filtfilt');
set(supt,'FontSize',18,'FontWeight','bold','Color','k')
set(gcf,'units','normalized','outerposition',[0 0 1 1]);
%%
% cd('D:\NEC lab\Ear-Biosignal\Figure3\SSVEP_all\SSVEP_ipsil')
...
figure(1),saveas (gcf, 'SSVEP_ipsil_com.jpg')
figure(2),saveas (gcf, 'SSVEP_ipsil_ind.jpg')
