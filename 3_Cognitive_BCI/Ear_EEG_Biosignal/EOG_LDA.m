...
for sub=1:15
    ...
    for trial=1:40
        for ch=1:9
            epoch_R{ch}(:,trial) = bp(start2+1+1600*(trial-1):start2+si+1600*(trial-1),10+ch);
            epoch_L{ch}(:,trial) = bp(start2+1+1600*(trial-1):start2+si+1600*(trial-1),ch);
        end
...
for sub=1:15
    ...
    fv{sub,1}(:,end) = [ones(10,1) ; ones(10,1)*2];
    fv{sub,2}(:,end) = [ones(10,1) ; ones(10,1)*2];
end
%%
YTrain = fv{1,1}(:,end);
YTrain2 = fv{1,2}(:,end);
