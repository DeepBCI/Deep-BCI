...
for sub=1:15
    ...
    % re-reference        
    for ch=1:9
        Rear_ch{ch}(:,sub) = bp(start+250:start+15249,10+ch,:);
        Lear_ch{ch}(:,sub) = bp(start+250:start+15249,ch,:);
    end
    ...
    for ch=1:9
        ...
        rms_task(sub,ch+9) = mean(rms(task_L{ch}));
        rms_rest(sub,ch) = mean(rms(rest_R{ch}));
        rms_rest(sub,ch+9) = mean(rms(rest_L{ch}));
    end
    rms_task(sub,19) = mean(rms(task_Chin));
    rms_rest(sub,19) = mean(rms(rest_Chin));
...
% label1 = {'ก่','','ก่','','ก่','','ก่','','ก่','','ก่','','ก่','','ก่','','ก่','','ก่','',''};
label1 = {'ก่','ก่','ก่','ก่','ก่','ก่','ก่','ก่','ก่','ก่','', 'SNR'};
...
for ch=1:9
    ...
    ax2.XAxis.Limits = [0 19000]; ax2.YAxis.Limits = [0 8]; ax2.YTick = [0 4 8]; hold on;
    ax2.XTick = 17000; ax2.XTickLabel = 'SNR';
    bar(17000,SNR_gavr(1,ch+9),1500,'FaceColor',[0.9290 0.6940 0.1250]);
    errorbar(17000,SNR_gavr(1,ch+9),NaN,SNR_err(1,ch+9),'k')
    text(15800,SNR_gavr(1,ch+9)+SNR_err(1,ch+9)+0.5,num2str(round(SNR_gavr(1,ch+9),2)),'FontWeight','bold','FontSize',8)
end
...
ax2.XAxis.Limits = [0 19000]; ax2.YAxis.Limits = [0 8]; ax2.YTick = [0 4 8]; hold on;
ax2.XTick = 17000; ax2.XTickLabel = 'SNR';
bar(17000,SNR_gavr(1,19),1500,'FaceColor',[0.9290 0.6940 0.1250]);
errorbar(17000,SNR_gavr(1,19),NaN,SNR_err(1,19),'k')
text(15800,SNR_gavr(1,19)+SNR_err(1,19)+0.5,num2str(round(SNR_gavr(1,19),2)),'FontWeight','bold','FontSize',8)
supt = suptitle('Chin : No re-reference');
set(supt,'FontSize',18,'FontWeight','bold','Color','k')
set(gcf,'units','normalized','outerposition',[0 0 1 1]);

%%
cd('D:\NEC lab\Ear-Biosignal\Figure\Figure4\Chin_Gavr_2s')
figure(1),saveas (gcf, 'Gavr_no_SNR.jpg')

%% RMS
% close

% label1 = {'ก่','','ก่','','ก่','','ก่','','ก่','','ก่','','ก่','','ก่','','ก่','','ก่','',''};
label1 = {'ก่','ก่','ก่','ก่','ก่','ก่','ก่','ก่','ก่','ก่','', 'RMS'};
...
for ch=1:9
    ...
    ax2.XAxis.Limits = [0 19000]; ax2.YAxis.Limits = [0 80]; ax2.YTick = [0 40 80]; hold on;
    ax2.XTick = 17000; ax2.XTickLabel = 'RMS';
    bar(17000,rms_gavr(1,ch+9),1500,'FaceColor',[0.9290 0.6940 0.1250]);
    errorbar(17000,rms_gavr(1,ch+9),NaN,rms_err(1,ch+9),'k')
    text(15500,rms_gavr(1,ch+9)+rms_err(1,ch+9)+5,num2str(round(rms_gavr(1,ch+9),2)),'FontWeight','bold','FontSize',8)
end
...
ax2.XAxis.Limits = [0 19000]; ax2.YAxis.Limits = [0 80]; ax2.YTick = [0 40 80]; hold on;
ax2.XTick = 17000; ax2.XTickLabel = 'RMS';
bar(17000,rms_gavr(1,19),1500,'FaceColor',[0.9290 0.6940 0.1250]);
errorbar(17000,rms_gavr(1,19),NaN,rms_err(1,19),'k')
text(15500,rms_gavr(1,19)+rms_err(1,19)+5,num2str(round(rms_gavr(1,19),2)),'FontWeight','bold','FontSize',8)
supt = suptitle('Chin : No re-reference');
set(supt,'FontSize',18,'FontWeight','bold','Color','k')
set(gcf,'units','normalized','outerposition',[0 0 1 1]);
...
%%
cd('D:\NEC lab\Ear-Biosignal\Figure\Figure4\Chin_Gavr_2s')
figure(2),saveas (gcf, 'Gavr_no_RMS.jpg')
