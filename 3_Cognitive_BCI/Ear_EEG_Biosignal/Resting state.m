...
for sub=1:15
    ...
    for ch=1:9
        Rear_ch{ch} = ECEO(1:15050,10+ch,:);
        Lear_ch{ch} = ECEO(1:15050,ch,:);
    end
    for ch=1:3
    ...
    for ch=1:9
        ...
        SNR(sub,ch+9) = mean(ECpL_ch{sub,ch},'all')/mean(EOpL_ch{sub,ch},'all');
    end
    for ch=1:3
        SNR(sub,ch+18) = mean(ECpS_ch{sub,ch},'all')/mean(EOpS_ch{sub,ch},'all');
    end
...
caxis(yscale); 
hb = colorbar('location','eastoutside','Ticks',[0 yscale(1,2)/2 yscale(1,2)],'FontSize',9,'FontWeight','bold','Color','k');
...
for ch=1:9
    ...
    ax2.YAxis.Visible = 'on'; ax2.XAxis.Visible = 'off';
    ax2.XAxis.Limits = [0 80]; ax2.YAxis.Limits = [0 7]; ax2.YTick = [0 3.5 7]; hold on;
    bar(70,SNR_avr(1,ch+9),8,'FaceColor',[0.9290 0.6940 0.1250]);
    errorbar(70,SNR_avr(1,ch+9),SNR_err(1,ch+9),SNR_err(1,ch+9),'k')
    text(62.5,SNR_avr(1,ch+9)+SNR_err(1,ch+9)+0.5,num2str(round(SNR_avr(1,ch+9),2)),'FontWeight','bold','FontSize',8)
    ax2.FontSize = 8; ax2.FontWeight = 'bold'; ax2.YColor = 'k';
end
caxis(yscale2); 
hb2 = colorbar('location','westoutside','Ticks',[0 yscale2(1,2)/2 yscale2(1,2)],'FontSize',9,'FontWeight','bold','Color','k');
...
for ch=1:3
    ...
    ax2.YAxis.Visible = 'on'; ax2.XAxis.Visible = 'off';
    ax2.XAxis.Limits = [0 80]; ax2.YAxis.Limits = [0 7]; ax2.YTick = [0 3.5 7]; hold on;
    bar(70,SNR_avr(1,ch+18),8,'FaceColor',[0.9290 0.6940 0.1250]);
    errorbar(70,SNR_avr(1,ch+18),SNR_err(1,ch+18),SNR_err(1,ch+18),'k')
    text(62.5,SNR_avr(1,ch+18)+SNR_err(1,ch+18)+0.5,num2str(round(SNR_avr(1,ch+18),2)),'FontWeight','bold','FontSize',8)
    ax2.FontSize = 8; ax2.FontWeight = 'bold'; ax2.YColor = 'k';
end
