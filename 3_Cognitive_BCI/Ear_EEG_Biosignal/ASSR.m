...
% %             stop_data = filtfilt(b,a,rawdata2{k}.data);
    ...
    %%    
    R_mean = mean(Hzs(1:3750,11:19,:),2);
    L_mean = mean(Hzs(1:3750,1:9,:),2);
    
    for ch=1:9
        Rear_ch{ch} = Hzs(1:3750,10+ch,:);
        Lear_ch{ch} = Hzs(1:3750,ch,:);
    end
    ...
    SNR(sub,19) = pCz_avr(6,sub)/mean(pCz_avr([1:5,7:11],sub),1); SNR(sub,20) = pPz_avr(6,sub)/mean(pPz_avr([1:5,7:11],sub),1);
end
...
% SNR_avr2(1,19) = pCz_gavr(6,1)/mean(pCz_gavr([1:5,7:11],1),1);
% SNR_avr2(1,20) = pPz_gavr(6,1)/mean(pPz_gavr([1:5,7:11],1),1);
...
% %     ax2.XAxis.Limits = [0 10]; ax2.YAxis.Limits = [0 6]; ax2.YTick = [0 3 6]; hold on;
    ...
    text(7.5,SNR_avr(1,ch+9)+SNR_err(1,ch+9)+0.5,num2str(round(SNR_avr(1,ch+9),2)))
end
subplot(5,9,23);
...
ax2.YAxis.Visible = 'on'; ax2.XAxis.Visible = 'off';
ax2.XAxis.Limits = [0 10]; ax2.YAxis.Limits = [0 6]; ax2.YTick = [0 3 6]; hold on;
bar(8.3,SNR_avr(1,19),1,'FaceColor',[0.9290 0.6940 0.1250]);
errorbar(8.3,SNR_avr(1,19),NaN,SNR_err(1,19),'k')
text(7.5,SNR_avr(1,19)+SNR_err(1,19)+0.5,num2str(round(SNR_avr(1,19),2)))
...
ax2.YAxis.Visible = 'on'; ax2.XAxis.Visible = 'off';
ax2.XAxis.Limits = [0 10]; ax2.YAxis.Limits = [0 6]; ax2.YTick = [0 3 6]; hold on;
bar(8.3,SNR_avr(1,20),1,'FaceColor',[0.9290 0.6940 0.1250]);
errorbar(8.3,SNR_avr(1,20),NaN,SNR_err(1,20),'k')
text(7.5,SNR_avr(1,20)+SNR_err(1,20)+0.5,num2str(round(SNR_avr(1,20),2)))
% % suptitle('ASSR : No re-reference - filtfilt, CF-1000Hz, MF-40Hz')
set(gcf,'units','normalized','outerposition',[0 0 1 1]);
%%
% cd('D:\NEC lab\Ear-Biosignal\Figure3\ASSR\ASSR_no')
...
% figure(1),saveas (gcf, 'ASSR_no_com.jpg')
% figure(2),saveas (gcf, 'ASSR_no_ind.jpg')
