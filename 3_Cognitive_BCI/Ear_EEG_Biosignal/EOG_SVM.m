...
for sub=1:15
    ...
    for trial=1:40
        for ch=1:9
            epoch_R{ch}(:,trial) = bp(start2+1+1600*(trial-1):start2+size+1600*(trial-1),10+ch);
            epoch_L{ch}(:,trial) = bp(start2+1+1600*(trial-1):start2+size+1600*(trial-1),ch);
        end
        epoch_ver(:,trial) = bp(start2+1+1600*(trial-1):start2+size+1600*(trial-1),34);
        epoch_hor(:,trial) = bp(start2+1+1600*(trial-1):start2+size+1600*(trial-1),35);
    end
...
for sub=1:15
    ...
    fv{sub,1}(:,21) = [ones(10,1) ; ones(10,1)*2];
    fv{sub,2}(:,21) = [ones(10,1) ; ones(10,1)*2];
end
...
%             [clasify, acc(i)] = EOG_SVM(fv{sub});
            [clasify, acc(i),YPredic(:,i)] = EOG_SVM2(fv{sub,1});
            C = confusionmat(YTrain,YPredic(:,i));
            CM(:,:,i) = C/10*100;
            
            [clasify2, acc2(i),YPredic2(:,i)] = EOG_SVM2(fv{sub,2});
