...
class VocabularyDeveloperFeedback(MainloopFeedback):
    ...
    def init(self):
        ...
        if self.hostname == 'sokrates':
            self.st_path = '/extra/data/bbciRaw/'
        else:
            # self.st_path = 'D:/Labworks/data/'
            self.st_path = ''
            self.st_path = 'C:\\demo_paradigms/pyff/src/Feedbacks/memory/db'
            #self.st_path = 'C:\\BBCI_toolbox\\pyff\\src\\Feedbacks\\memoryFeedback\\db'
            #self.st_path = 'D:\\Workspace\\Labworks\\RSVP Stuff\\pyff\\src\\Feedbacks\\memory\\db'
        #self.store_path = ''
        ...
        ##Yes, this dictates the number of words to learn per session.
...
#         self.store_logfile = ''.join([self.store_path, self.logfilename])
    ...
    def retrieve_word_list(self, username):
        words = self.db_query('select word_index from word_list where username = ? order by sessionid desc limit 60')#180 originally
    ...
    def play_tick(self):
        ...
        if self.typSes == 'Train_A' or self.typSes == 'Train_B' or self.typSes =='Train_C':
            ...
            if data != 1:
                ...
                print "b4 dict size",len(self.dictionary)
                print "b4 dict indice",len(self.dict_indices)
                ...
                print 'data log empty'

                "log is empty (first experiment)"
                self.init_lection2(os.path.join(self.lection_path, self.Filenames))

                print 'length(dic): ',len(self.dictionary), 'length(index): ', len(self.dict_indices)
            ...
            self.c.execute('select wl.word_index from word_list wl where wl.username = ? and wl.sessionid IN (select ss.sessionid from session ss where ss.username = ? and ss.session_type != "Test" order by ss.sessionid desc limit 3 ) order by wl.timestamp desc',(self.username, self.username))
...
#            self.dictionary = self.dictionary[0:self.max_dict_length]
    ...
    def update_bag(self, pair_to_remove):
        ...
        print 'bag_(before update):' , self.bag
        ...
        if self.learnable_word_count < self.learn_words-10:
                        ...
                        for event in pygame.event.get():
                            if (event.type == pygame.KEYUP):
                                ...
                                if(_ == '1'):
                                    ...
                                    print 'knwon word added, current index list is: ', self.known_pair_index
            ...
            print "learnable_word_count ", self.learnable_word_count
    ...
    def training_oneShw(self):
        ...
        while current_bag_idx <10:
            ...
            print "self.curr_bag_index is ", current_bag_idx
            print "self.bag_index is ", self.bag_index[current_bag_idx]
            print "self.dict+indice is ", self.dict_indices.index(self.bag_index[current_bag_idx])
            ...
            print "bag index", self.bag_index
            print "tmp", tmp

            self.do_print(['Did you already know this word?','1. Yes    2. No','please press 1. yes or 2. no'],size_list=[None, None, 30])
                ...
                for event in pygame.event.get():
                    if (event.type == pygame.KEYUP):
                        ...
                        # if the entered key is alphabetic it is part of the answer
                            ...
                            answer.append(_)
                        self.do_print(['Did you already know this word?', "".join(answer),'please press 1. yes or 2. no'],size_list=[None,None,30])
...
#            print 'self.bag= '+str(self.bag)
            if len(self.showed_sequence) < self.show_display_limit+1:
                print 'shown_seq= '+str(self.showed_sequence)
            else:
                print 'shown_seq= '+str(self.showed_sequence[-1*self.show_display_limit:])
...
#                       #elif(_.isalpha()):
    ...
    def testing(self):
        ...
        if self.typSes != 'Test':
            ...
            print "dict_ref : ", len(self.dictionary_reference)

            for item in test_indices:
                test_dictionary.append(self.dictionary_reference[item-1])
            print "session test? ", len(test_indices)

            self.dict_indices = test_indices

        else:
            print "dict_indices? ", len(self.dict_indices)
        ...
        print "dict size", len(self.dictionary)
...
#                random.shuffle(test_dictionary)
        print 'shuffled index ',temp_indices
        ...
        for i in temp_indices:
            ...
            "************************************************"
            print str(temp_indices.index(i)+1), ': '
...
####THIS ONE IS USED INSTEAD
    ...
    def create_db(self):
        ...
        print "sessionid is "+str(self.sessionid)
    ...
    def add_response(self, last_event_key, word_index=None, user_response=None, rightorwrong=None):
        self.db_query('insert into user_answer(event_key, sessionid, username, word_index, user_response, rightorwrong) values(?,?,?,?,?,?)',
