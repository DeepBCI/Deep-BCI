...
def build_convpool_conv1d(input_vars, nb_classes, imsize=32, n_colors=3, n_timewin=3, n_filter=64):
    """
...
def build_convpool_lstm(input_vars, nb_classes, grad_clip=110, imsize=32, n_colors=3, n_timewin=3):
    ...
    convpool = tf.add(tf.matmul(convpool[:, -1], weight_variable([n_hidden, 256])), bias_variable([256])) #slice layer
    convpool = tf.nn.relu(convpool)
    convpool = tf.nn.dropout(convpool, 0.5)

    convpool = tf.add(tf.matmul(convpool, weight_variable([256, nb_classes])), bias_variable([nb_classes]))
    return convpool
...
def train(images, labels, fold, model_type, batch_size=32, num_epochs=5, n_layer = (4, 2, 1)):
    ...
    with tf.Session(config = config) as sess:
        saver.restore(sess, filesave_path)
        print("Model restored: ", model_type)
