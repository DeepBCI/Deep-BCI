...
save_direc = 'chan_by_chan_ebe_sta12_MAXREL'
...
if __name__ == '__main__':
    ...
    for cc in range(64):
        ...
        for i in range(np.asarray(MBlist).shape[0]):
            print('-' * 10, ' : [', i + 1, '|', np.array(MBlist).shape[0], ']', '-' * 10)
            mbep=ext_epochs('ALPHA/sta1',MBlist,i)
            mfep=ext_epochs('ALPHA/sta1',MFlist,i)
            epo_mb = channel_selection(mbep,cc)
            epo_mf = channel_selection(mfep,cc)

            mbep2=ext_epochs('ALPHA/sta2',MBlist2,i)
            mfep2=ext_epochs('ALPHA/sta2',MFlist2,i)
            ...
            print('3. saving')
            start_time = time.time()
            filename1 = save_direc + '/channel_{0}'.format(cc) + '/' + MBlist[i][:5]  + '_specflex_MI'
