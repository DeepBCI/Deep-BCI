...
cnt = proc_filtButter(cnt, 5, [25 125]);
...
cnt.title= ['dir\_word1_eeg_to_mat'];
...
for ii = 1:length(mrk.pos)
    concat_EEG(:,:,ii) = EEG_data(mrk.pos(1,ii)-2300:mrk.pos(1,ii),:); % time series x channel x trial %% lengh of speech 2000ms but 500ms for bl
end
